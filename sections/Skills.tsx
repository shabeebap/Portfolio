...
const CategoryBlock = ({ category }: { category: typeof skillCategories[0] }) => {
  const { ref, isInView } = useInViewAnimation<HTMLDivElement>({ triggerOnce: true, rootMargin: "-50px" });
    ...
    <div ref={ref} className="flex flex-col items-center">
      ...
      <motion.h3
        ...
        className={cn(
          ...
          category.highlight ? "text-accent drop-shadow-[0_0_10px_rgba(var(--accent),0.4)]" : "text-foreground/40"
        )}
      >
        {category.title}
      </motion.h3>
              ...
              >
                <motion.div
                  ...
                  className={cn(
                    ...
                    isHighlight
                      ? "px-8 py-4 md:px-10 md:py-5 text-xl md:text-3xl font-black bg-accent/10 border-2 border-accent/50 text-accent shadow-[0_0_30px_rgba(var(--accent),0.2)] hover:bg-accent/20 hover:border-accent hover:shadow-[0_0_50px_rgba(var(--accent),0.6)]"
  ...
  return (
    <section id="skills" className="relative py-32 bg-background overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 text-sm text-accent bg-accent/10 rounded-full border border-accent/20 mb-6 font-semibold tracking-wide shadow-[0_0_15px_-3px_rgba(var(--accent),0.2)]">
